...
import { getFamilyWithCode } from "./family";
...
export async function createIndividualForFamilyUsingCode(
  code: string,
  newIndividual: FamilyIndividual,
): Promise<FamilyIndividual | null> {
  try {
    const familyID = await getFamilyWithCode(code);
...
export async function attachUserToFamilyIndividual(
  userID: number,
